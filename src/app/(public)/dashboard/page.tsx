...
export default async function DashboardPage() {
  ...
  const [upcomingDraw, pastTickets] = await Promise.all([
    getNextUpcomingDraw(),
    getUserTickets(userId),
  ]);

  const currentTicket = upcomingDraw
    ? await getUserTicketForDraw(userId, upcomingDraw.id)
    : null;
            ...
            <CardContent className="flex flex-wrap items-center justify-between gap-6 p-6">
              <div>
                ...
                <p className="text-xl font-semibold">
                  {formatDate(upcomingDraw.drawDate)}
                </p>
                <p className="mt-1 text-primary font-semibold">
                  {formatCurrency(upcomingDraw.prizeAmount)} prize pool
                </p>
              </div>
